...
import {
  Search, Inbox, Loader2, MessageCircle, CheckCircle2, XCircle, ArrowRightCircle, Send, Trash2,
} from 'lucide-react'
...
import {
  ...
  getRequestStatusHistory, acceptRequestAndConvert, REQUEST_STATUSES, getQuotationById, deleteRejectedRequests,
} from '../utils/db.js'
      ...
      <div className="space-y-5">
        <Card className="!p-4 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2 shrink-0">
            ...
            {rejectedCount > 0 && (
              <button onClick={() => setConfirmingClear(true)} className="btn-danger shrink-0">
                <Trash2 className="h-4 w-4" /> Clear Rejected ({rejectedCount})
              </button>
            )}
          </div>
        </Card>
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[900px]">
                ...
                <tbody>
                  {filtered.map((r) => (
                    <tr key={r.id} onClick={() => setSelected(r)} className="border-b border-slate-100 last:border-0 hover:bg-primary-50/40 cursor-pointer">
                      <td className="px-4 py-3 font-mono text-xs text-slate-500">{r.requestNumber}</td>
                      <td className="px-4 py-3 font-medium text-slate-800">{r.fullName}</td>
                      <td className="px-4 py-3 text-slate-600">{r.programmeName}</td>
                      <td className="px-4 py-3 text-slate-600 whitespace-nowrap">{r.aggregate ? `${r.aggregate}/400` : (r.jambScore ?? '-')}</td>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button className="btn-danger" onClick={handleClearRejected} disabled={clearing}>
              {clearing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />} Clear {rejectedCount} Request(s)
            </button>
          </>
        }
      >
        <p className="text-sm text-slate-600">
          This permanently deletes every request currently marked REJECTED (useful for clearing out test data). This cannot be undone.
        </p>
      </Modal>
    </DashboardLayout>
  )
      ...
      <div className="space-y-5 text-sm">
        ...
        <Section title="Assessment">
          ...
          {request.jambSubjects?.length > 0 && (
            <p className="text-xs text-slate-500 mt-2"><strong>JAMB Subjects:</strong> {request.jambSubjects.join(', ')}</p>
          )}
          {request.olevelSubjects?.length > 0 && (
            <p className="text-xs text-slate-500 mt-1"><strong>O'Level:</strong> {request.olevelSubjects.map((s) => `${s.subject} (${s.grade})`).join(', ')}</p>
          )}
        </Section>
        ...
        <Section title="Programme &amp; Package">
          ...
          <p className="text-xs text-slate-600 mt-2">{request.recommendation}</p>
        </Section>
          ...
          <Section title="Payment">
            {linkedQuotation ? (
              linkedQuotation.paid ? (
                <Grid>
                  <Field label="Payment Status" value="Fully Paid" />
              ...
              ) : linkedQuotation.paidAmount > 0 ? (
                <p className="text-xs text-amber-600">Partially paid: {formatCurrency(linkedQuotation.paidAmount, settings.currency_symbol)} of {formatCurrency(linkedQuotation.price, settings.currency_symbol)}.</p>
              ) : (
                <p className="text-xs text-amber-600">Not yet paid. Record payment from Checkout &amp; Invoices.</p>
              )
            ) : (
              <p className="text-xs text-slate-400">Loading payment info...</p>
            )}
            <div className="glass-panel p-3 text-xs text-emerald-700 mt-3">
...
function Section({ title, children }) {
  return (
    <div>
      <p className="text-xs font-bold uppercase tracking-wide text-primary-700 mb-2">{title}</p>
      {children}
    </div>
  )
}
